...
elseif strcmp(t.Properties.VariableNames{1},'fileID')

    for i = 1:height(t)
        vd = VoxelData(t.fullpath{i});
        ...
        if cog.x>1 && t.Move_or_keep_left(i)
            vd.mirror;
            printwhendone{end+1} = ['____mirrored: ',t.fullpath{i}];
        else
            printwhendone{end+1} = ['not_mirrored: ',t.fullpath{i}];
        end
